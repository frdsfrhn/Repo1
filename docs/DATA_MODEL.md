...
| `propertyImageUrl` | string | Optional — download URL for a one-pager/flyer photo (`property_images/{agentId}/{leadId}/one_pager.jpg` in Storage), shown on the dashboard tile and lead detail |
| `ownerName` / `ownerPhone` | string | Optional — the property owner/landlord, distinct from `prospectName`/`phoneNumber` (the client). Not in the original locked data model; added for a WhatsApp-the-owner action on lead detail |
